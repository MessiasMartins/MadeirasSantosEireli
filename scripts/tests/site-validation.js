...
function run() {
  ...
  assert(siteStructure.includes("{ href: '/produtos', title: 'Produtos' }"), 'Header sem link para /produtos.')
  assert(siteStructure.includes("{ href: '/entrega', title: 'Entrega' }"), 'Header sem link para /entrega.')
  assert(siteStructure.includes("{ href: '/contato', title: 'Contato' }"), 'Header sem link para /contato.')
  ...
  assert(seoComponent.includes("'@type': 'BreadcrumbList'"), 'Structured data de breadcrumb não encontrado.')
  ...
  assert(pageMetadata.includes('Contato e Localização | Madeiras Santos'), 'Metadata de Contato ausente.')

  assert(siteStructure.includes("href: '/produtos/telhas'"), 'Estrutura de navegação sem rota de telhas.')
  assert(
  ...
  assert(!read('data/authors/default.md').includes('Madeiras Santos EIRELI'), 'Autor ainda contém EIRELI.')
  ...
  ;[
    ...
    assert(sitemap.includes(`<loc>https://www.madeirassantos.com.br${route}</loc>`), `Sitemap sem rota ${route}`)
  })

  assert(robots.includes('Sitemap: https://www.madeirassantos.com.br/sitemap.xml'), 'robots.txt sem sitemap correto.')
